...
#include <pthread.h> // compared to Boost, frickin' lightweight!! :D
...
int compare_weights(const void *kp1, const void *kp2) {
	return ((Keypoint*)kp2)->response - ((Keypoint*)kp1)->response > 0 ? 1 : -1;
}

int compare_px(const void *kp1, const void *kp2) {
	return ((Keypoint*)kp1)->px - ((Keypoint*)kp2)->px > 0 ? 1 : -1;
}

double cyclic_dist(double s, double t) {
	return s-t - round((s-t)/(2*PI)) * 2 * PI;
}

// Prominency is a number between 1 and 0, where 0 is the strongest feature,
// 1 is the weakest feature. It measures the stronger one of the pair.
int compare_dists(Keypoint *kp1, Keypoint *kp2, double prominency) {
