...
-- | Helper jacobian
jacob_h :: Camera -> Feature -> Matrix Float
jacob_h (Camera (xc, yc) phic) (Feature f _) = fromLists 
...
jacobian :: Camera -> Feature -> Matrix Float
...
initialize (Camera (posx,posy) angle) (phi, varphi) = Feature (fromList [posx, posy, angle+atan(phi), rho0]) cov where
	rho0 = 0.1
